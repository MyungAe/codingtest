...
def solution():
    # 큰 팁을 먼저 받는게 이득. why? 어차피 순서로 인해 빠지는 금액은 동일함.

    # get tips
    customer_number = int(readline())

    customer_tips = []
    for _ in range(customer_number):
        customer_tips.append(int(readline()))

    # sort tips
    customer_tips.sort(reverse=True)

    # sum tips
    total_tips = 0
    for index in range(customer_number):
        current_tip = customer_tips[index] - index
        if current_tip > 0:
