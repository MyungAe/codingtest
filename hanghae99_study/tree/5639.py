...
def solution():
    ...
    def post_order(start, end):
        ...
        root_node = preorder_list[start]
        bigger_index = start + 1

        while end >= bigger_index:
            if preorder_list[bigger_index] > root_node:
                break
            bigger_index += 1

        post_order(start + 1, bigger_index - 1)
        post_order(bigger_index, end)

        print(root_node)

    post_order(0, len(preorder_list) - 1)
