...
def solution():
    ...
    for _ in range(repeat_count):
        filename, file_extension = readline().split('.')
        # print(filename, file_extension)
        if file_extension not in obj:
            # print('file extension is not found')
            obj[file_extension] = 0
        obj[file_extension] += 1

    # print(sorted(obj))

    for extension in sorted(obj):
        print(extension, obj[extension])
