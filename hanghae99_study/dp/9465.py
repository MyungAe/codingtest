...
def solution():
    ...
    for _ in range(test_case_count):
        list_length = int(readline())

        zero_row_stickers_score = list(map(int, readline().split()))
        one_row_stickers_score = list(map(int, readline().split()))

        dp = [zero_row_stickers_score, one_row_stickers_score]
        # print(dp)
        ...
        for index in range(2, list_length):
            dp[0][index] += max(dp[1][index - 1], dp[1][index - 2])
            dp[1][index] += max(dp[0][index - 1], dp[0][index - 2])
            print(dp)

        print(max(dp[0][list_length - 1], dp[1][list_length - 1]))
